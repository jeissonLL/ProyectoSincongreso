...
class Congreso extends basedatos{
...
public function crear_congreso() {
    $datos   =  "INSERT INTO tbl_congreso(nombre_congreso, siglas, descripcion_congreso, lugar, coordenadas,id_pais_fk,
       logo_congreso,  lema,  numero_cai, anio,  fecha_inicio,  fecha_finalizacion, fecha_i_recepcion, fecha_f_recepcion,
     fecha_i_revision, fecha_f_revision,  fecha_p_programa,fecha_cambio_costo_inscripcion,
    id_estado_congreso_fk, creado_por, fecha_creacion) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $insert = $this->base->prepare($datos);
    $arrData = array($this->nombre_congreso, $this->siglas, 
    $this->descripcion_congreso, $this->lugar, $this->coordenadas, $this->id_pais_fk,$this->logo_congreso,
     ...
     $this->fecha_p_programa, $this->fecha_cambio_costo_inscripcion, 2 , $this->idusuario_cm, date("y-m-d") , TRUE);
    $resInsert = $insert->execute($arrData);
    $idInsert = $this->base->lastInsertId();
    if(   $idInsert   >   0   ) {
        $this->id_congreso_pk   =   (int)$idInsert;
        self::subir_logo_congreso();
        return true;
    }
    else {
...
/*
    ...
    public function crear_congreso(){
        ...
        $sql = insert("INSERT INTO tbl_congreso(nombre_congreso, siglas, descripcion_congreso, lugar, coordenadas, id_pais_fk, logo_congreso, lema, numero_cai, anio, fecha_inicio, fecha_finalizacion, fecha_i_recepcion, fecha_f_recepcion, fecha_i_revision, fecha_f_revision, fecha_p_programa, fecha_cambio_costo_inscripcion, id_estado_congreso_fk, creado_por, fecha_creacion) VALUES (:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?,:?)");
        $insert= $this->base->prepare($sql);
        $arrData = array($this->nombre_congreso, $this->siglas, $this->descripcion_congreso, $this->lugar, $this->coordenadas, $this->id_pais_fk, "", $this->lema, $this->numero_cai, $this->anio, $this->fecha_inicio, $this->fecha_finalizacion, $this->fecha_i_recepcion, $this->fecha_f_recepcion, $this->fecha_i_revision, $this->fecha_f_revision, $this->fecha_p_programa, $this->fecha_cambio_costo_inscripcion, 2, $this->idusuario_cm, date("y-m-d"), TRUE);
        $resInsert = $insert->execute($arrData);
        $idInsert = $this->base->lastInsertId();
        return $idInsert;
   ...
   */ 
    public function editar() {
        $consulta          =   $this->base->select("SELECT `logo_congreso` FROM `tbl_congreso` WHERE `id_congreso_pk` = ".$this->id_congreso_pk."");
        ...
        $datos             =   $this->base->update("UPDATE `tbl_congreso` SET `nombre_congreso`=?,`siglas`=?,`descripcion_congreso`=?,`lugar`=?,`coordenadas`=?,`id_pais_fk`=?,`logo_congreso`=?,`lema`=?,`numero_cai`=?,`anio`=?,`fecha_inicio`=?,`fecha_finalizacion`=?,`fecha_i_recepcion`=?,`fecha_f_recepcion`=?,`fecha_i_revision`=?,`fecha_f_revision`=?,`fecha_p_programa`=?,`fecha_cambio_costo_inscripcion`=?,`fecha_modificacion`=?,`modificado_por`=? WHERE `id_congreso_pk`=".$this->id_congreso_pk."", "sssssssssssssssssssi", [$this->nombre_congreso, $this->siglas, $this->descripcion_congreso, $this->lugar, $this->coordenadas, $this->id_pais_fk, $nombre_logo, $this->lema, $this->numero_cai, $this->anio, $this->fecha_inicio, $this->fecha_finalizacion, $this->fecha_i_recepcion, $this->fecha_f_recepcion, $this->fecha_i_revision, $this->fecha_f_revision, $this->fecha_p_programa, $this->fecha_cambio_costo_inscripcion, date("y-m-d"), $this->idusuario_cm]);
        $this->base->insert("INSERT INTO tbl_log(id_usuario_fk, fecha, hora, ip, informacion_extra, id_tipo_log_fk) VALUES(?,?,?,?,?,?)", "issssi", [$this->idusuario_cm, date("y-m-d"), date('H:i:s'), $_SERVER['REMOTE_ADDR'], "Modificación de información básica del Congreso ".$this->nombre_congreso." con id ".$this->id_congreso_pk, 1]);
        if(   $datos   >   0 || $datos == 0   ) {
            self::subir_logo_congreso();
            return true;
        }
    ...
    public function activar() {
        ...
        {
            $activar   =   $this->base->update("UPDATE `tbl_congreso` SET `id_estado_congreso_fk`=?,`fecha_modificacion`=?,`modificado_por`=? WHERE `id_congreso_pk`=".$this->id_congreso_pk."", "isi", [1, date("y-m-d"), $this->idusuario_cm]);
            $log       =   $this->base->insert("INSERT INTO tbl_log(id_usuario_fk, fecha, hora, ip, informacion_extra, id_tipo_log_fk) VALUES(?,?,?,?,?,?)", "issssi", [$this->idusuario_cm, date("y-m-d"), date('H:i:s'), $_SERVER['REMOTE_ADDR'], "Activó el congreso con id: ".$this->id_congreso_pk."", 2]);
            return true;
        }
        else {
            return false;
    ...
    public function cerrar() {
        if(  self::congreso_activo()  ) {
            $activar   =   $this->base->update("UPDATE `tbl_congreso` SET `id_estado_congreso_fk`=?,`fecha_modificacion`=?,`modificado_por`=? WHERE `id_congreso_pk`=".$this->id_congreso_pk."", "isi", [3, date("y-m-d"), $this->idusuario_cm]);
            $log       =   $this->base->insert("INSERT INTO tbl_log(id_usuario_fk, fecha, hora, ip, informacion_extra, id_tipo_log_fk) VALUES(?,?,?,?,?,?)", "issssi", [$this->idusuario_cm, date("y-m-d"), date('H:i:s'), $_SERVER['REMOTE_ADDR'], "Cerró el congreso con id: ".$this->id_congreso_pk."", 2]);
            return true;
        }
        else {
            return false;
    ...
    public function eliminar() {
        $registros_lineas       =      $this->base->select("SELECT `id_linea_investigacion_pk` FROM `tbl_congreso_linea_investigacion` WHERE `id_congreso_pk` = ".$this->id_congreso_pk);
        $lineas                 =      array();
        $contador_lineas        =      0;
        $contador_congreso      =      0;
        $validacion             =      false;
        ...
        for(   $i   =   0;   $i   < sizeof($lineas);   $i++   ) {
            $linea_borrada      =      $this->base->delete("DELETE FROM `tbl_linea_investigacion` WHERE `id_linea_investigacion_pk`=".$lineas[$i]);
            if(   $linea_borrada      ==   1   ) {
                $contador_lineas++;
            }
        }
        $congreso_borrado       =   $this->base->delete("DELETE FROM `tbl_congreso` WHERE `id_congreso_pk`=".$this->id_congreso_pk);
    ...
    private function tiene_organizadores() {
        $consulta     =   $this->base->select("SELECT COUNT(`id_institucion_fk`) FROM `tbl_organizadores` WHERE `id_congreso_fk` = ".$this->id_congreso_pk);
        $arreglo      =   mysqli_fetch_row($consulta);
    ...
    private function tiene_lineas() {
        $consulta     =   $this->base->select("SELECT COUNT(`id_linea_investigacion_pk`) FROM `tbl_congreso_linea_investigacion` WHERE `id_congreso_pk` = ".$this->id_congreso_pk."");
        $arreglo      =   mysqli_fetch_row($consulta);
        $contador     =   (int)$arreglo[0];
        $validacion   =   ($contador > 0)   ?   true  :  false;
        return $validacion;
    ...
    private function congreso_activo() {
        $consulta          =   $this->base->select("SELECT `id_estado_congreso_fk` FROM `tbl_congreso` WHERE `id_congreso_pk`=".$this->id_congreso_pk);
        $arreglo           =   mysqli_fetch_row($consulta); 
